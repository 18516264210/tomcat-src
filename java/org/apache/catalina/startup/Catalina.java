...
public class Catalina {
    ...
    protected Digester createStartDigester() {
        ...
        //如果解析到<Server>标签的时候，就创建一个org.apache.catalina.core.StandardServer对象
        //详细信息可参考http://tyrion.iteye.com/blog/1912290
    ...
    public void load() {
        ...
        System.out.println("==>"+getClass()+",load()");
        ...
        // Create and execute our Digester
        System.out.println("==>"+getClass()+",createStartDigester()，创建server.xml解析器");
        ...
        try {
            try {
                System.out.println("==>"+getClass()+"，获取配置文件file:conf/server.xml");
                file = configFile();
                System.out.println("==>"+getClass()+"，读取配置文件file:conf/server.xml");
            ...
            if (inputStream == null) {
                System.out.println("==>"+getClass()+"，server.xml配置文件为空，通过getResourceAsStream()方式读取");
                try {
            ...
            try {
                System.out.println("==>"+getClass()+"，将server.xml文件流添加到inputSource对象");
                inputSource.setByteStream(inputStream);

                //一是创建一个Digester对象，将当前对象压入Digester里的对象栈顶，
                // 根据inputSource里设置的文件xml路径及所创建的Digester对象所包含的解析规则生成相应对象，

                System.out.println("==>"+getClass()+"，将"+this+"加入到digester");
                digester.push(this);

                System.out.println("==>"+getClass()+"，digester解析inputSource对象，开始###############################");
                digester.parse(inputSource);
                System.out.println("==>"+getClass()+"，digester解析inputSource对象，完毕###############################");
        ...
        System.out.println("==>"+getClass()+"，将"+this+"设置到server中，开始");
        getServer().setCatalina(this);
        System.out.println("==>"+getClass()+"，将"+this+"设置到server中，结束");
        // Stream redirection
        initStreams();
        ...
        try {
            ...
            System.out.println("==>"+getClass()+",调用"+getServer().getClass()+"的init()方法，开始##############################");
            //这里开始调用StandardServer的init()方法，但是StandardServer中并没有init()方法，所以这里的init()实际上是LifecycleBase类的init()方法
            //调用完成后，有调用了initInternal()方法，这个方法是StandardServer类的方法。居然杀了个回马枪。
            getServer().init();
            System.out.println("==>"+getClass()+",调用"+getServer().getClass()+"的init()方法，结束##############################");
        } catch (LifecycleException e) {
    ...
    public void start() {
        ...
        // Start the new server
        try {
            System.out.println("==>"+getClass()+"，开始调用Server的start方法");
        ...
        // 判断服务器是否进入等待状态
        System.out.println("==>"+getClass()+",判断是否进入等待状态："+await);
        if (await) {
            await();
            stop();
        }
