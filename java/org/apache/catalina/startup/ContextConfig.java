 ...
 *
 * 解析web.xml，并将servlet配置信息放置在不同的map中。
 * 同时将jsp文件解析为servlet
 */
public class ContextConfig implements LifecycleListener {
     ...
     * 事件驱动，触发监听
     *
     *              Tomcat中的Lifecycle是怎么回事?
     *              总的来说就是通过一个工具类LifecycleSupport，
     *              调用该类的addLifecycleListener方法增加监听器，
     *              需要发布事件时还是调用该工具类的fireLifecycleEvent方法，
     *              将事件发布给组件上注册的所有监听器，由监听器内部实现来决定是否处理该事件。
     */
    @Override
    public void lifecycleEvent(LifecycleEvent event) {
        ...
        if (event.getType().equals(Lifecycle.CONFIGURE_START_EVENT)) {
            System.out.println("==>"+getClass()+",Lifecycle.CONFIGURE_START_EVENT=configure_start，调用解析方法");
            // 解析 Web.xml 配置文件
            System.out.println("==>"+getClass()+"，调用configureStart()，解析配置文件");
            configureStart();
    ...
    protected synchronized void configureStart() {
        ...
        // 解析 web.xml 配置文件
        System.out.println("==>"+getClass()+",webConfig()方法，解析web.xml");
        webConfig();
    ...
    protected void webConfig() {
        ...
        System.out.println("==>"+getClass()+"，读取web.xml开始");
        InputSource contextWebXml = getContextWebXmlSource();
        System.out.println("==>"+getClass()+"，读取web.xml结束");

        System.out.println("==>"+getClass()+"，解析web.xml开始【包括servlet，filter，listener等信息】");
        parseWebXml(contextWebXml, webXml, false);
        System.out.println("==>"+getClass()+"，解析web.xml结束");

        System.out.println("====>遍历配置所有servlet开始");

        Map<String, ServletDef> servlets = webXml.getServlets();
        for (String s : servlets.keySet()) {
            System.out.println("==>"+getClass()+"，servletName:"+s+",servletDef:"+servlets.get(s).getServletClass());
        }

        System.out.println("====>遍历配置所有servlet结束");
