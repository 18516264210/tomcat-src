...
public class JIoEndpoint extends AbstractEndpoint<Socket> {
                ...
                try {
                    ...
                    if (running && !paused && setSocketOptions(socket)) {
                        ...
                        if (!processSocket(socket)) {
                            ...
                            log.info("==>关闭socket连接1");
                            closeSocket(socket);
                        }
                    } else {
                        countDownConnection();
                        // Close socket right away
                        log.info("==>关闭socket连接2");
                        closeSocket(socket);
                    }
                } catch (IOException x) {
                    if (running) {
                ...
                try {
                    ...
                    if ((state != SocketState.CLOSED)) {// 非关闭状态
                        if (status == null) {
                            ...
                            log.info("==>"+getClass()+",handler.process(0)");
                            state = handler.process(socket, SocketStatus.OPEN_READ);
                        } else {
                            log.info("==>"+getClass()+",handler.process(1)");
    ...
    protected boolean processSocket(Socket socket) {
        ...
        try {
            ...
            log.info("==>"+getClass()+"开启一个任务处理浏览器请求");
