 ...
 *
 * 文件解析器，通过下面的方法进行加载类，同时将具有生命周期的类加入到监听中
...
public class Digester extends DefaultHandler2 {
        ...
        if ((rules != null) && (rules.size() > 0)) {
            for (int i = 0; i < rules.size(); i++) {
                try {
                    ...
                    //通过反射的形式加载类，并加入到监听中
                    System.out.println("==>"+getClass()+"，这里通过反射的形式生成对象，namespaceURI:"+namespaceURI+",name:"+name+",list:"+list);
                    rule.begin(namespaceURI, name, list);
                } catch (Exception e) {
     ...
     * @exception SAXException if a parsing exception occurs
     *
     * 这里加载类是通过反射的形式进行加载，同时将实现了或者间接实现了lifecycle接口的类加入到监听中
     * org.apache.tomcat.util.digester.ObjectCreateRule的begin()方法中
     */
    public Object parse(InputSource input) throws IOException, SAXException {
 
        configure();
        //这里产生对象，具体实现类，在ObjectCreateRule中的begin()方法
        getXMLReader().parse(input);
        return (root);
